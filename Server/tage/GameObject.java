...
public class GameObject
{
	...
	*/
	...
	/** calculates the distance between this object and another specified object */
	public float distance(GameObject obj) {
		Vector3f loc1 = this.getWorldLocation();
		Vector3f loc2 = obj.getWorldLocation();
		float dx = loc2.x() - loc1.x();
		float dy = loc2.y() - loc1.y();
		float dz = loc2.z() - loc1.z();
		dx *= dx;	dy *= dy;	dz *= dz;
		float sum = dx + dy + dz;
		return (org.joml.Math.sqrt(sum));
	}

	/** calculates the distance between this object and a specified location */
	public float distance(Vector3f loc2) {
		Vector3f loc1 = this.getWorldLocation();
		float dx = loc2.x() - loc1.x();
		float dy = loc2.y() - loc1.y();
		float dz = loc2.z() - loc1.z();
	...
	/** yaws the GameObject by a fraction of the inputted value */
	public void yaw(float rotVal, float elapsedTime) {
		Matrix4f rotAroundAvatarUp, newRotation;
		newRotation = new Matrix4f(worldRotation);
		rotAroundAvatarUp = new Matrix4f().rotation((-rotVal*elapsedTime)/450f, new Vector3f(0, 1, 0));
		newRotation.mul(rotAroundAvatarUp);
		setLocalRotation(newRotation);
	}

	/** pitches the GameObject by a fraction of the inputted value */
	public void pitch(float rotVal, float elapsedTime) {
		Matrix4f rotAroundAvatarUp, newRotation;
		newRotation = new Matrix4f(worldRotation);
		rotAroundAvatarUp = new Matrix4f().rotation((-rotVal*elapsedTime)/750f, new Vector3f(1, 0, 0));
		newRotation.mul(rotAroundAvatarUp);
		setLocalRotation(newRotation);
	}

	/** rolls the GameObject by a fraction of the inputted value */
	public void roll(float rotVal, float elapsedTime) {
		Matrix4f rotAroundAvatarUp, newRotation;
		newRotation = new Matrix4f(worldRotation);
		rotAroundAvatarUp = new Matrix4f().rotation((-rotVal*elapsedTime)/600f, new Vector3f(0, 0, 1));
		newRotation.mul(rotAroundAvatarUp);
		setLocalRotation(newRotation);
	}
}