...
public class Camera
{
	...
	/** yaws the Camera by a fraction of the inputted value */
	public void yaw(float rotVal, float elapsedTime) {
		setU(new Vector3f(getU().rotateAxis((-rotVal*elapsedTime)/450f, v.x(), v.y(), v.z())));
		setN(new Vector3f(getN().rotateAxis((-rotVal*elapsedTime)/450f, v.x(), v.y(), v.z())));
	}

	/** pitches the Camera by a fraction of the inputted value */
	public void pitch(float rotVal, float elapsedTime) {
		setV(new Vector3f(getV().rotateAxis((rotVal*elapsedTime)/750f, u.x(), u.y(), u.z())));
		setN(new Vector3f(getN().rotateAxis((rotVal*elapsedTime)/750f, u.x(), u.y(), u.z())));
	}

	/** pitches the Camera by a fraction of the inputted value */
	public void roll(float rotVal, float elapsedTime) {
		setU(new Vector3f(getU().rotateAxis((rotVal*elapsedTime)/600f, n.x(), n.y(), n.z())));
		setV(new Vector3f(getV().rotateAxis((rotVal*elapsedTime)/600f, n.x(), n.y(), n.z())));
	}
